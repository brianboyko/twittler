  ...
  var showTweets = function(interval) {
    ...
    while (streams.home.length > 0) {
      ...
      $tweet.html('<div class="profile">&nbsp;</div>#' +
        globalTweetNumber + '   @' + '<a class="userlink" id="' +
        uniqueID + '" href="#">' + tweet.user +
        '</a>: <div class="message">' + tweet.message +
        '</div><div class="timeStamp">' + tweet.created_at + '</div>');
  ...
  var prepUserTweet = function() {
    ...
    $('#userTweet').keypress(function(event) {
      if (event.which == 13 || event.keycode == 13) {
        ...
        myUserTweet = {};
        $('#userTweet').val('');
        $('#userTweet').attr('placeholder',
          'Tweet as many times as you like!');
      } // endif
